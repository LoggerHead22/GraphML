...
class Dataset(DGLDataset):
    ...
    def __getitem__(self, index):
        smiles, y = self.dataset[self.indexes[index]]
        graph_obj = smiles2graph(smiles)
        
        graph = dgl.graph((list(graph_obj['edge_index'][0]), list(graph_obj['edge_index'][1])), 
                          num_nodes =graph_obj['num_nodes'])

        graph.ndata['feat'] = torch.from_numpy(graph_obj['node_feat'])
        graph.edata['feat'] = torch.from_numpy(graph_obj['edge_feat'])
...
def train(model, device, loader, optimizer):
    ...
    for step, (bg, labels) in enumerate(tqdm_note(loader, position=0, leave=True, desc="Iteration")):
        bg = bg.to(device)
        x = bg.ndata.pop('feat')
...
def eval(model, device, loader, evaluator):
    ...
    return evaluator.eval(input_dict)["mae"], r2
...
def test(model, device, loader):
    ...
    for step, (bg, _) in enumerate(tqdm_note(loader, position=0, leave=True, desc="Iteration")):
        bg = bg.to(device)
        x = bg.ndata.pop('feat')
        edge_attr = bg.edata.pop('feat')
...
def main():
    ...
    if args.train_subset:
        ...
        train_loader = DataLoader(Dataset(dataset, subset_idx), batch_size=args.batch_size, shuffle=True,
                                  num_workers=args.num_workers, collate_fn=collate_dgl)
    else:
        #train_loader = DataLoader(dataset[split_idx["train"]], batch_size=args.batch_size, shuffle=True,
        train_loader = DataLoader(Dataset(dataset, split_idx["train"]), batch_size=args.batch_size, shuffle=True,
                                  num_workers=args.num_workers, collate_fn=collate_dgl)

    valid_loader = DataLoader(Dataset(dataset, split_idx["valid"]), batch_size=args.batch_size, shuffle=False,
                              num_workers=args.num_workers, collate_fn=collate_dgl)

    if args.save_test_dir != '':
        test_loader = DataLoader(Dataset(dataset, split_idx["test"]), batch_size=args.batch_size, shuffle=False,
                                 num_workers=args.num_workers, collate_fn=collate_dgl)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print('Evaluating...')
        valid_mae, valid_r2 = eval(model, device, valid_loader, evaluator)
