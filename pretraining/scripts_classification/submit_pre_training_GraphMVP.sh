...
export SSL_masking_ratio_list=(0.15)
...
for dropout_ratio in "${dropout_ratio_list[@]}"; do
     ...
     echo "$(( gpustat | head -n $(( 2 + 2 )) | tail -n 1 | cut -d "|" -f 3 | tr -d '[:space:]' | cut -d '/' -f 1 ) 2>&1)"

     #until [ $((gpustat | head -n $(( 2 + 2 )) | tail -n 1 | cut -d "|" -f 3 | tr -d '[:space:]' | cut -d '/' -f 1 ) 2>&1) -gt 5  ]
     #     do 
     #     echo "Wait free gpu $device"
     #     sleep 60
     #done
     ...
     --output_model_dir="$output_model_dir" #> output_file
