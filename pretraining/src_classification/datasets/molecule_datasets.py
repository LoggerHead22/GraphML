...
def mol_to_graph_data_obj_with_3D(mol):
    """ used in MoleculeDataset() class
    Converts rdkit mol objects to graph data object in pytorch geometric
    NB: Uses simplified atom and bond features, and represent as indices
    :param mol: rdkit mol object
...
def gen_data(i, smiles_list, rdkit_mol_objs):
    rdkit_mol = rdkit_mol_objs[i]
    if rdkit_mol is None:
        return None
    try:
        data = mol_to_graph_data_obj_with_3D(rdkit_mol)
    except Exception as e:
        print(e, smiles_list[i])
        return None
    return data
...
class MoleculeDataset(InMemoryDataset):
    ...
    def process(self):
        print(self.raw_dir, self.raw_paths, self.raw_paths[0], self.dataset)
        def shared_extractor(smiles_list, rdkit_mol_objs, labels):
            ...
            #    _ids = 0
            #    for data in tqdm.tqdm(pool.imap(partial(gen_data, smiles_list=smiles_list, rdkit_mol_objs=rdkit_mol_objs), iterable=range(len(smiles_list)), 
            #                                                chunksize=10)):
            #        if data is None:
            #            bad_mol_count += 1
            #            continue
            ...
            for i in tqdm.tqdm(range(len(smiles_list))):
                ...
                try:
                    ...
                    data.mol_id = torch.tensor([i])
                    data.y = torch.tensor(labels[i])
                    data_list.append(data)
                    data_smiles_list.append(smiles_list[i])
                    _ids += 1

            print("Bad Mol Count:", bad_mol_count)
