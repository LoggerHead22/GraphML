...
class GNN3D(nn.Module):
    def __init__(self, gnn_model, model_3d):
        super(GNN3D, self).__init__()
        self.model_2d = gnn_model
        self.model_3d = model_3d
    
    def forward(self, x, edge_index, edge_attr, positions, batch, return_2d_repr = True):
        node_2d_repr = self.model_2d(x, edge_index, edge_attr)
        molecule_3D_repr, node_3d_repr = self.model_3d(node_2d_repr, positions, batch)
        node_2d_repr += node_3d_repr
        if return_2d_repr:
            return molecule_3D_repr, node_2d_repr
        else:
            return molecule_3D_repr
