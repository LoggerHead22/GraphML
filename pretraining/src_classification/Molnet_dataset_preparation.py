...
class Molecule3DMolNetDataset(InMemoryDataset):

    def __init__(self, root, n_mol, n_conf, n_upper, transform=None, seed=777, dataset_name='toxcast',
        ...
        super(Molecule3DMolNetDataset, self).__init__(
    ...
    def process(self):
        ...
        downstream_task_list = [self.dataset_name]
        ...
        if self.smiles_copy_from_3D_file is None:  # 3D datasets
            ...
            for smiles, sub_dic in tqdm(drugs_summary):
                smiles = smiles.strip()
                if smiles not in whole_SMILES_set:
                    continue
                if sub_dic.get('pickle_path', '') == '':
                    notfound += 1
                    continue
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_name', type=str, help='number of unique smiles/molecules')
    ...
    if args.sum:
        ...
        dataset_name, n_conf, n_upper = args.dataset_name, args.n_conf, args.n_upper
        ...
        root_2d = '{}/molecule_datasets/{}/{}_2D_nconf{}_nupper{}'.format(data_folder, dataset_name, dataset_name, n_conf, n_upper)
        root_3d = '{}/molecule_datasets/{}/{}_3D_nconf{}_nupper{}'.format(data_folder, dataset_name, dataset_name, n_conf, n_upper)


        # Generate 3D Datasets (2D SMILES + 3D Conformer)
        Molecule3DMolNetDataset(root=root_3d, n_mol=100, n_conf=n_conf, n_upper=n_upper, dataset_name=dataset_name)
        # Generate 2D Datasets (2D SMILES)
        Molecule3DMolNetDataset(root=root_2d, n_mol=100, n_conf=n_conf, n_upper=n_upper, dataset_name=dataset_name,
