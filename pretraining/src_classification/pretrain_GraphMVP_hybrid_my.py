...
from models import (GNN, GNN3D ,AutoEncoder, Discriminator, SchNet, MySchNet,
...
from util import (ExtractSubstructureContextPair, MaskAtom, NegativeEdge,
                  do_GraphCL, do_GraphCLv2, dual_CL,
...
def train_no_aug(args, device, loader, optimizer):
    ...
    for batch in l:
        ...
        molecule_2D_3D_repr, node_repr = molecule_model_2D(batch.x, batch.edge_index, batch.edge_attr, batch.positions, batch.batch)
        molecule_2D_repr = molecule_readout_func(node_repr, batch.batch)
        #molecule_3D_repr = molecule_model_3D(batch.x[:, 0], batch.positions, batch.batch)
        ...
        if args.SSL_2D_mode == 'EP':
            ...
            _, masked_node_repr = molecule_model_2D(batch.masked_x, batch.edge_index, batch.edge_attr, batch.positions, batch.batch)
...
if __name__ == '__main__':
    ...
    if args.SSL_2D_mode in ['GraphCL', 'JOAO', 'JOAOv2']:
        dataset = MoleculeGraphCLMaskingDataset(data_root, dataset=args.dataset, mask_ratio=args.SSL_masking_ratio)
    ...
    molecule_model_2D = GNN3D(molecule_model_2D_, molecule_model_3D_)

    molecule_model_3D = nn.Linear(1, 1)
